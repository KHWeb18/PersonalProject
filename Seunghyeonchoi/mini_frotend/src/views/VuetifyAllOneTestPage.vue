<template>
    <div>
            ...
            <v-dialog v-model="dialog" persistent max-width="400">
                ...
                <v-card>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn color="teal darken-1" text @click.native="btn_click($event)">결제승인</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
        </v-layout>
        ...
        <v-dialog v-model="loginDialog" persistent max-width="500px">
            ...
            <v-card>
                ...
                <v-card-actions>
                    <v-spacer></v-spacer>
                    <v-btn color="teal darken-1" text @click="btnLoginClick($event)">취소</v-btn>
                    <v-btn color="teal darken-1" text @click="btnLoginClick($event)">확인</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
    ...
    <!-- 게시판 목록이 더러우면 container로 감싸주면 깔끔해진다 -->
    <v-container>
    <!-- :header=" 뜻: v-bind가 생략돼있음 -->
    <!-- headerTitle는 제목 -->
    <!-- :items는 매핑할 것(?) -->
    ...
    <div class="hide_btn">되냐</div>
  <div class="hide_btn" :class="{ on : isHideImage }">되네</div>
  <v-btn @click="isHideImage = !isHideImage">버튼</v-btn>
...
export default {
    ...
    data () {
        return {
            search: '',
            headerTitle: [
                { text: '번호', value: 'boardNo', width: '80', align: 'center' },
                { text: '제목', value: 'title', width: '500', align: 'center' },
                { text: '작성자', value: 'writer', width: '200', align: 'center' },
                { text: '내용', value: 'content', width: '100', align: 'center' }
            ],
            contents: [
                { boardNo: 1, title: '놀면뭐하니? 국비로 무료교육받는 방법!(언리얼게임/3D프린터)', writer: '으마으마', content: '으아아아앜' },
