...
export default {
  ...
  computed: {
    ...mapState([ 'togetherComments', 'togetherComment' ])
  },
  mounted () {
    this.fetchTogetherComment(this.commentNo)
  },
  methods: {
    ...mapActions([ 'fetchTogetherComment', 'fetchTogetherComments' ]),
    btnCancle () {
      this.dialog = false
    },
    modifyComment () {
      const content  = this.content
      axios.patch(`http://localhost:7777/together/comment/modify/${this.commentNo}`, { content }).then(() => {
        alert('수정이 완료되었습니다')
    ...
    readComment () {
      this.fetchTogetherComment(this.commentNo)
      this.content = this.togetherComment.content
    }
  }
}
</script>