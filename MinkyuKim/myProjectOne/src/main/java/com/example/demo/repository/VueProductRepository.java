...
public class VueProductRepository {
    ...
    public void create(Product product) throws Exception {
        String query = "insert into vueproduct (product_name, description, producer, price, files) values (?, ?, ?, ?, ?)";

        jdbcTemplate.update(query, product.getProduct_name(), product.getDescription(),
                product.getProducer(), product.getPrice(), product.getFiles());
    }

    public List<Product> list() throws Exception {

        List<Product> results = jdbcTemplate.query(
                "select product_no, product_name, description, producer, price, files, reg_date from vueproduct " +
    ...
    public Product read (Integer productNo) throws Exception {
        List<Product> results = jdbcTemplate.query(
                "select product_no, product_name, description, producer, price, files," +
    ...
    public void update(Product product) throws Exception {
        String query = "update vueproduct set product_name = ?, description = ?," +
                "price = ?, files = ? where product_no = ?";

        log.info("Vueproduct Repository: " + product);

        jdbcTemplate.update(query, product.getProduct_name(), product.getDescription(),
                product.getPrice(), product.getFiles(), product.getProductNo());
    }
